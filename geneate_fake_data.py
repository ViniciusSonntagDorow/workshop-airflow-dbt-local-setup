        ...
        for chunk_start in range(0, tamanho_lote, chunk_size):
            ...
            for _ in range(chunk_start, chunk_end):
                try:
                    ...
                    tem_desconto = (
                        random.random() < 0.2
                    )  # 20% de chance de ter desconto
                    valor_desconto = (
                        round(valor_total * random.uniform(0.05, 0.2), 2)
                        if tem_desconto
                        else 0.0
                    )
                    ...
                    cupom = (
                        f"CUPOM{str(uuid.uuid4())[:8].upper()}"
                        if tem_desconto
                        else None
                    )
...
def main():
    ...
    try:
        ...
        for i in range(0, total_pedidos, lote_pedidos):
            print(
                f"Processando pedidos {i + 1}-{min(i + lote_pedidos, total_pedidos)}..."
            )
